...
public interface Vd_Cp_Loja_Virt_Repository extends JpaRepository<VendaCompraLojaVirtual, Long> {
	...
	@Query(value="select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
			+ " where i.vendaCompraLojaVirtual.excluido = false and i.vendaCompraLojaVirtual.id = ?1")
	...
	@Query(value="select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
			...
			+ " and upper(trim(i.vendaCompraLojaVirtual.pessoa.cpf)) like %?2%")
	...
	@Query(value="select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
			+ " where i.vendaCompraLojaVirtual.excluido = false and upper(trim(i.vendaCompraLojaVirtual.enderecoCobranca.ruaLogradouro)) "
			+ " like %?1%")
	List<VendaCompraLojaVirtual> vendaPorEndereCobranca(String enderecocobranca);

	
	@Query(value="select distinct(i.vendaCompraLojaVirtual) from ItemVendaLoja i "
			+ " where i.vendaCompraLojaVirtual.excluido = false and upper(trim(i.vendaCompraLojaVirtual.enderecoEntrega.ruaLogradouro)) "
			+ " like %?1%")
